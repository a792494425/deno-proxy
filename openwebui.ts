...
async function handleWebSocketRequest(
  ...
  try {
    ...
    // --- 健壮的双向事件绑定 ---
    const forwardMessage = (source: WebSocket, destination: WebSocket, direction: string) => {
    ...
    forwardMessage(clientSocket, serverSocket, '客户端 -> 服务器');
    forwardMessage(serverSocket, clientSocket, '服务器 -> 客户端');
    ...
    // 返回 101 Switching Protocols 响应给客户端
    return response;
...
async function startServer() {
  // 从 Deno.args 和 Deno.env 解析配置
  const args = parse(Deno.args);
  const port = Number(args.port || args.p) || Deno.env.get("PORT") || DEFAULT_PORT;
  const targetHost = String(args.host || args.h) || Deno.env.get("TARGET_HOST") || DEFAULT_TARGET_HOST;
  const targetScheme = String(args.scheme) || Deno.env.get("TARGET_SCHEME") || DEFAULT_TARGET_SCHEME;
  const allowedOrigin = String(args.origin) || Deno.env.get("ALLOWED_ORIGIN") || "*";
